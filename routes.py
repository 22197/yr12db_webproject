...
def hardware(id):
    conn = sqlite3.connect('HARDWARE.db')
    cur = conn.cursor()
    # Query --> get all the items in table Hardware, MediaType and ConsoleType
    cur.execute('''SELECT
                    h.hw_id,
                    h.hw_name,
                    h.release_yr,
    ...
    conn.close()
    return render_template("hardware.html", title="hardware", id=id,
                            hardware=hardware, softwareseries=softwareseries)
